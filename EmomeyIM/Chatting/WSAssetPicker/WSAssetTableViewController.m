...
#import "WSVideoEditController.h"

#define ASSET_WIDTH_WITH_PADDING 104.0f
...
- (void)viewDidLoad{
   ...
   // rightItem.tintColor = [UIColor whiteColor];
    ...
    self.tableView.contentInset = UIEdgeInsetsMake(2, 0, 2, 0);
...
- (NSInteger)assetsPerRow{
    return MAX(1, (NSInteger)floorf(self.tableView.contentSize.width / (ASSET_WIDTH_WITH_PADDING * ([[UIScreen mainScreen] bounds].size.width) / 320.0)));
}
...
- (void)assetsTableViewCell:(WSAssetsTableViewCell *)cell didSelectAsset:(BOOL)selected atColumn:(NSUInteger)column{
    ...
    NSUInteger assetIndex = indexPath.row * self.assetsPerRow + column;
    WSAssetWrapper *assetWrapper = [self.fetchedAssets objectAtIndex:assetIndex];
    if (self.assetPickerState.pickingVideo) {
        WSVideoEditController *controller = [[WSVideoEditController alloc] init];
...
//#define ROW_HEIGHT  (79.0f * (ScreenWidth) / 320)
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
	return (104.0f * ([[UIScreen mainScreen] bounds].size.width) / 320.0);
}
