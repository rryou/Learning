...
- (id)initWithGIFFile:(NSString*)gifFilePath {
   return  [self initWithGIFFile:gifFilePath withAnimationRepeatCount:0];
}
- (id)initWithGIFFile:(NSString*)gifFilePath  withAnimationRepeatCount:(int)count
{
    ...
    return [self initWithGIFData:imageData WithAnimationRepeatCount:_rePeatCount];
}
...
- (id)initWithGIFData:(NSData*)gifImageData WithAnimationRepeatCount:(int)count
{
    _rePeatCount = count;
...
{
    return [self initWithGIFData:gifImageData WithAnimationRepeatCount:0];
}
...
- (void)loadImageData {
	...
	UIImage *firstImage = [array firstObject];
	...
	[self setImage:[overlayArray firstObject]];
    self.lastImage = [overlayArray lastObject];
    ...
    if (1 == _rePeatCount )
    {
        [self performSelector:@selector(setImageAtAfterRepeatOne) withObject:nil afterDelay:(total - oneDelay -1)/100];
    }
	[self startAnimating];
}

- (void)setImageAtAfterRepeatOne
{
    [self stopAnimating];
    if (lastImage)
    {
