...
- (UIToolbar *)createInputHelperToolBar{
    ...
    UIBarButtonItem *btnPrevious = [[UIBarButtonItem alloc]initWithTitle:@"" style:UIBarButtonItemStyleDone target:self action:@selector(didClickButtonPrevious)];  // 上一项
    UIBarButtonItem *btnNext = [[UIBarButtonItem alloc]initWithTitle:@"" style:UIBarButtonItemStyleDone target:self action:@selector(didClickButtonNext)];  // 下一项
...
//    NSLog(@"keyboardWillShow %f", _currentRootView.frame.origin.y);
    
//    NSLog(@"setupInputHelperForView %@ %@", NSStringFromCGRect(view.frame), NSStringFromCGRect(view.bounds));
...
//        NSLog(@"keyboardWillShow 2 %f", _currentRootView.frame.origin.y);
    }
...
{
    if ([getAssociatedObjectForKey(_currentRootView, kInputHelperRootViewOriginalOriginY) floatValue] == _currentRootView.frame.origin.y) {
    ...
    else
    {
        ...
        [UIView animateWithDuration: 0.2f animations:^{
            //        NSLog(@"_currentRootViewY 2 %@", NSStringFromCGRect(frame));
