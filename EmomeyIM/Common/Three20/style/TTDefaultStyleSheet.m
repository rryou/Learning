...
- (TTStyle*)linkText:(UIControlState)state {
  if (state == UIControlStateHighlighted) {
    return
      ...
      [TTInsetStyle styleWithInset:UIEdgeInsetsMake(3, 4, 3, 4) next:
      [TTTextStyle styleWithColor:self.linkTextColor next:
      [TTUnderlineStyle styleWithColor:self.linkTextColor width:1 next:nil]]]]]];
...
- (TTStyle*)searchTextField {
  return
    ...
    [TTSolidFillStyle styleWithColor:[(TTDefaultStyleSheet *)[TTStyleSheet globalStyleSheet] backgroundColor ]next:
...
//  return RGBCOLOR(87, 107, 149);
    return RGBCOLOR(0x00, 0x61, 0xfb);
}
