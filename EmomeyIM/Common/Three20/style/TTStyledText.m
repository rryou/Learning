...
- (void)drawAtPoint:(CGPoint)point highlighted:(BOOL)highlighted textColor:(UIColor*)textColor
{
    ...
    if (maxnumOfLines == 1)
    {
        ...
        while (frame)
        {
            if(sumWith <= maxWith )
            {
                [frame drawInRect:frame.bounds withTextColor:textColor];
                frame = frame.nextFrame;
                sumWith += frame.width;
            }
            else
...
{
    [self drawAtPoint:point highlighted:highlighted textColor:nil];
}
